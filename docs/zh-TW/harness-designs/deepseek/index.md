...
這項設計最大的好處是：**大量功能根本不必修改 agent 循環本身**。想在工具執行前做安全檢查？監聽 `tools/pre-execute`。想加入記憶？在 `agent/pre-step` 注入。想記錄行為？訂閱 session 事件。想修改模型請求？掛接 `agent/request`。想決定是否繼續推理？監聽 `agent/turn-stopping`。
